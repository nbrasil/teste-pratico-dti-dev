...
exports.createPedido = async (req, res) => { 
    ...
    try {
        //Buscar todos os obstáculos do banco de dados
        const obstaculos = await dbAll('SELECT * FROM obstaculos');

        // Verificar se o ponto do pedido colide com algum obstáculo
        for (const obs of obstaculos) {
            // fórmula da distância entre dois pontos: sqrt((x2-x1)² + (y2-y1)²)
            const distancia = Math.sqrt(Math.pow(localizacao_x - obs.x, 2) + Math.pow(localizacao_y - obs.y, 2));
            
            if (distancia < obs.raio) {
                //Se colidir, retorna um erro 400 (Bad Request) com uma mensagem específica
                return res.status(400).json({ error: `Localização inválida: dentro da zona de exclusão "${obs.nome}".` });
            }
        }

        // Se não houver colisão, prossegue com a criação do pedido
